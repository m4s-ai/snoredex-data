...
def printing_semantic_key(release_id: str, printing: dict[str, Any]) -> bytes:
    normalized_markings(printing.get("markings"))  # Retain the collector's validation boundary.
    return graph_printing_semantic_key(release_id, printing).encode("utf-8")
...
def printing_semantic_core_key(release_id: str, printing: dict[str, Any]) -> bytes:
    ...
    normalized_markings(printing.get("markings"))
    return graph_printing_semantic_key(release_id, printing, include_edition=False).encode("utf-8")
