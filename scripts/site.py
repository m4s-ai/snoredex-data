...
def main() -> int:
    ...
    # them as one block of "refuted claims" (#66). Only an owner adjudication settles a language/
    # printing absence; exhaustive source scopes can support its rationale. The rest are one
    # source's disagreement, and
    ...
    open_claims_note = (
        f"<li><strong>{open_units} legacy claims remain open.</strong> They are shown as "
        f"unresolved rather than quietly dropped. This is queue depth inside the frozen candidate "
        f"universe, not a count of undiscovered local printings.</li>" if open_units else ""
    )
...
<section id="about">
  ...
  <div class="callout">
    ...
    <ul>
      ...
      <li><strong>Finish evidence is positive-first.</strong> A complete official manifest may close
      alternatives within its exact scope — currently {finish_counts['withCompleteManifest']} units
      — and a separate owner finish decision may close a reviewed unit without introducing a finish.</li>
...
<section id="methodology">
  ...
  <ul>
    ...
    <li>For language/printing claims, only an explicit collection-owner adjudication makes a final
    absence decision after reviewing all cited evidence. Complete official scopes provide bounded
    rationale; they are not themselves the decision authority. Finish alternatives use their own
    closure model: a complete official checklist may close its exact scope.</li>
  </ul>
