...
def main() -> int:
    ...
    # The page used to state its own resolution sentence by hand, and the copy said "All 719
    # claims are now resolved" twice over a duplicated line. Generate it, and let it describe the
    # queue it actually finds — a reopened claim should change this sentence, not outlive it.
    unresolved = counts["pending"] + counts["needs-manual-review"]
    resolution = (
        f"All {total_claims} inherited language/product claims are resolved"
        if not unresolved else
        f"{total_claims - unresolved} of {total_claims} inherited language/product claims are "
        f"resolved"
    )
    status_body = f"""  <p class="lede">
      Every claim is one card × one language × one variant, checked against a source outside
      Cardmarket. This page reports review status only inside the frozen legacy Cardmarket
      candidate universe <code>{baseline['meta']['baselineId']}</code>. {resolution}; that is not
      a claim that every locality or printing was discovered.
    </p>
