...
def image_derivatives(src: str, content_hash: str | None) -> dict[str, str]:
    ...
    preview = ROOT / "images" / "previews" / f"{stem}.jpg"
    thumbnail = ROOT / "images" / "thumbs" / f"{stem}.jpg"
    if preview.is_file():
        result["previewSrc"] = f"images/previews/{preview.name}"
    if thumbnail.is_file():
        result["thumbnailSrc"] = f"images/thumbs/{thumbnail.name}"
    return result
...
def main() -> int:
    ...
    if "--check" in sys.argv:
        if (not OUT.exists() or OUT.read_text(encoding="utf-8") != rendered
                or not OUT_JS.exists() or OUT_JS.read_text(encoding="utf-8") != rendered_js):
