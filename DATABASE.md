...
| `app_language_availability` | Raw repository verdict, conservative application status, evidence and provider strength per claimed language. |
...
- `application_status='not-printed'` is reserved for evidence explicitly allowed to establish
  absence.
- `application_status='disputed'` preserves a repository contradiction whose provider is not
  absence-capable. Do not turn it into a hard “does not exist.”
...
The database is UTF-8 SQLite, schema version `1.0.0`, with `PRAGMA user_version=10000`. Every build
