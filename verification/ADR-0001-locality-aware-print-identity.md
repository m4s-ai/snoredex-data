...
[#145](https://github.com/m4s-ai/snoredex-data/issues/145); migration pending
...
This decision defines the identity boundary before any consumer reads new ids. It changes no
authoritative store. The machine-readable contract is
[`print_identity_schema.json`](print_identity_schema.json); the complete migration dry-run is
...
The c301933 dry-run made the second problem measurable: 634 confirmed language units became only
...
- Consumer migration and actual SQLite constraints land in #140. The schema records those future
  constraints now so migration cannot weaken the boundary.
...
- Source-first discovery can add a local release without inventing a Cardmarket product or work
  equivalence.
- Compatibility projections remain necessary until #140 migrates consumers.
