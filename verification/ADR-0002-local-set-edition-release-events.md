...
**Status:** accepted design, executable dry-run and authoritative graph migration completed by
...
This decision adds the catalogue layer above ADR-0001's card releases. The reviewed catalogue
nodes and provenance edges are materialized in `snoredex.sqlite`'s `graph_*` tables and committed
in [`authoritative_graph.json`](authoritative_graph.json); the raw input registry remains
immutable. The raw input registry is [`set_catalogue_sources.json`](set_catalogue_sources.json),
and the executable SQLite contract is [`set_catalogue_schema.sql`](set_catalogue_schema.sql).
...
The migration recorded these losses before #140. No compatibility field is allowed to become
evidence for a new catalogue node, and no compatibility projection is a live graph input.
