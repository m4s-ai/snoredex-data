...
nodes and provenance edges are materialized in `snoredex.sqlite`'s `graph_*` tables; the raw input
registry remains immutable and the compatibility app projection remains available. The raw input registry is
