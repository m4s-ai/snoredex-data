...
**Grade physical evidence explicitly, and by what the claim rests on.** `sourceType` distinguishes *photographed specimen* from *owner attestation*, and `providerId` must name the source the unit would fall over without — not the strongest source standing near it. **31 units rest on owner attestation alone**; 5 rest on a photographed specimen alone, each citing its `SPEC-nnnn` record. Check `E4` holds this sentence to the data, so correct it here rather than letting it drift (it read "currently 0" for weeks while the real figure was 16, then 30). A specimen may only be claimed by a unit that cites it — `S14` — and `sourceRef` holds a reference or nothing, never prose — `S13`.
